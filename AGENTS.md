...
M11.2 首屏布局与画板关闭回归计划见 `docs/FR_Imageprompt-M11.2-startup-board-close-plan.md`。图库从空列表起预留垂直滚动条轨道，首批图片和全量薄元数据必须使用相同视口宽度；无滚动内容时只隐藏轨道视觉，不取消占位。画板实际批准关闭后永久退休 Chrome，停止并解除计时器回调，迟到的客户区/非客户区鼠标和延时入口不能重新启动；坐标转换须验证 PresentationSource，不能仅用关闭后可能仍为真的 IsLoaded。保存失败取消关闭时不能退休。新增 `--m112-smoke` 仅允许 M11.2 合成图库，包含首屏渲染几何与关闭后迟到事件回归。验证/发布结果以该计划账本为准。

M11.1 画板全选便签、顶部栏悬停、参考锁定和多图排版已于 2026-09-08 完成，独立计划见 `docs/FR_Imageprompt-M11.1-board-selection-reference-layout-plan.md`。Ctrl+A/框选后的图片与便签选择在成员按下时保留，可共同移动/删除并一步撤销；混合选择不增加共同缩放/旋转职责。顶部感应区改为连续悬停 450ms（覆盖历史 250ms），离开取消、拖动不触发；必须同时处理 WPF 客户区和 WindowChrome 非客户区鼠标事件。Alt+Q 忽略重复按键，保持显示到再次切换或画布按下。Ctrl+L/顶部按钮/右键切换参考锁定，按图库路径+画板 ID 记入应用设置，保护图片与便签的修改、增删及历史操作，允许浏览/缩放/平移；不得只禁用按钮而漏掉直接指针、属性或外部加入入口。排版按旋转后外包围框提供六种对齐和双轴等间距，只移动所选图片，保留两端锚点，空间不足时不改动；每次有效排版一个撤销记录、一次批量保存。沿用 schema 13 与 M11 待保存保护。Debug/Release 415/415，最终 EXE 动画开/关均 44/44、M11 故障 20/20；160 次透明交接偏移/重排为 0，5,000 项画板 P95 16.671ms。记录见 `docs/performance/2026-09-08-m11.1-board-selection-reference-layout.json`，用户包为 `publish/FR_Imageprompt-M11.1-final-win-x64.zip`。

M11 稳定性、数据安全、剪贴板防误收录、透明交接和画板保存回归已于 2026-09-07 完成，计划与逐步账本见 `docs/FR_Imageprompt-M11-stability-data-safety-repair-plan.md`。修复前基线 `48875a9` 与修复提交 `e44277d` 均已推送。schema 13 新增撤销校验表，后续修改/原图后改时保守拒绝撤销；多文件或文件夹复制整次忽略且不反馈/不入收件箱；画板保留最新 50 步历史，保存失败保留待保存内容并支持 Ctrl+S，切板/关闭/托盘退出先保存，场景恢复同事务。透明交接先取得所有权再冻结输入/转移快照，取消会清理渲染等待并恢复来源，不得恢复重复请求取消有效转移的旧路径。Debug/Release 403/403、最终 EXE WPF 20/20、160 次透明交接偏移和重排均为 0，M7.2 与 M8.2～M8.5/M10 回归通过；记录见 `docs/performance/2026-09-07-m11-stability-data-safety.json`。既有性能和输入契约继续保持。
...
M9 离线签名、一证一机许可证的产品契约、密钥边界、逐步实施和验证账本维护在：

- `docs/FR_Imageprompt-M9-offline-device-license-blueprint.md`

M10 轻量注释节点、统一临时相机状态和主图库透明零重排的产品契约、顺序实施和验证账本维护在：

- `docs/FR_Imageprompt-M10-note-camera-transparent-stability-blueprint.md`

M10 已于 2026-08-30 完成。便签现为轻量注释/标题节点：不含字距、自动换行或输入期自动增高，样式通过现有 `color_style` 的版本化兼容载荷持久化，五个全局预设只进入应用设置，不得为其增加数据库 schema。`Space`、`Ctrl+Space`、`Ctrl+0`、图片双击和 `Esc` 共用一份首次临时视图快照；透明模式切换保留图库布局占位、列数、卡片尺寸和滚动锚点，切换本身重排为 0，只有透明模式下用户手工改窗结束后允许一次 180ms 防抖重排。Debug/Release 全量测试均为 359/359；性能与真界面记录见 `docs/performance/2026-08-30-m10-note-camera-transparent-stability.json`。

M10.1 便签焦点、相机快捷键与图库右边缘的体验回归蓝图维护在：

- `docs/FR_Imageprompt-M10.1-focus-camera-gallery-edge-regression-blueprint.md`

M10.1 已于 2026-08-30 完成。便签输入所有权改为沿视觉树识别所属根元素，非编辑态编辑器不再命中/聚焦，离焦时先同步退出编辑再异步保存；选择态 `Space` 采用同目标切换，`Ctrl+Space` 使用仅活动画板注册的系统热键规避中文输入法吞键，`Ctrl+0`/数字键盘 0 和 `Esc` 精确恢复继续共用首次快照。图库宽度以内部滚动视口为权威并保留 8 DIP 右边缘安全区，透明切换重排仍为 0。Debug/Release 构建 0 警告、0 错误，全量测试均为 364/364；M7.2 与 M8.2～M8.5 回归通过，记录见 `docs/performance/2026-08-30-m10.1-focus-camera-gallery-edge-regression.json`。后续不得用降低 M7.2 虚拟化/缓存标准、改变 M8 图片输入职责或恢复透明模式重排来修改这些行为。

M10.2 画板交互与透明窗口无闪交接的独立回归蓝图维护在：

- `docs/FR_Imageprompt-M10.2-board-interaction-transparent-handoff-blueprint.md`

M10.2 已于 2026-08-31 完成。默认等比拖角现使用固定原始对角线连续投影，240 次生产 WPF 更新期间视觉/位图重建和数据库写入均为 0；新便签默认 420×140 DIP、32px、16 DIP 垂直边距，图片/便签命中使用类型安全标识，颜色改为 WPF 常用色/HEX 浮层；无选择 `Space` 提供一层手工相机往返，顶部栏使用 `Alt+Q`、8 DIP/250ms 唤出和 500ms 安全隐藏。主图库透明模式采用隐藏预热、两帧稳定和原子窗口交接；0/20/50/90% 四位置双向共 160 次均为 0 偏移误差、0 重排，并通过退役窗口弱引用回收门禁。Debug/Release 全量测试均为 374/374，性能与数据安全记录见 `docs/performance/2026-08-31-m10.2-board-interaction-transparent-handoff.json`。后续不得恢复主导轴切换式缩放、旧窗口视觉树持有、裸 Alt/F10 顶栏入口或透明模式先显示首屏再恢复滚动的旧路径。
...
M9 已于 2026-08-11 完成：一份许可证绑定一台电脑，默认永久、可选一年，无有效许可证时在设置和图库初始化前完全阻止进入；V2 许可证允许全部 V2.x。用户程序只内置 ECDSA P-256 公钥，签发私钥保存在授权人 Windows 凭据管理器并支持口令认证加密备份，不得进入源码、Git、用户包、日志或诊断包。普通用户包、授权人签发器和个人许可证物理分离；M9 未修改图库数据库结构、M7.2 浏览性能层或 M8 画板交互。全量 Release 测试 346/346，原始记录见 `docs/performance/2026-08-11-m9-offline-device-license.json`。

新的 Codex 对话在进行功能开发、性能优化、UI 重构、无感收录、AI 元数据或自由画板工作前，必须先完整阅读适用蓝图。自由画板还必须完整阅读 M8、M8.2、M8.3、M8.4、M8.5 与 M10；实施时从适用蓝图中第一个未勾选步骤继续。每完成一步，必须在对应蓝图中勾选并追加日期、主要文件、验证结果和遗留问题。
