...
准备首次体验时，建议按[首次体验与问题反馈指南](docs/first-experience-guide.md)完成一轮测试。完整功能说明见[用户指南](docs/user-guide.md)，AI 数据边界见[本地与在线 AI 隐私](docs/privacy-and-ai.md)，模型包格式见[模型包文档](docs/model-pack.md)。
...
| 空格 | 主图库：打开纯图片沉浸大图；画板：聚焦所选内容或完整画板，再按一次精确恢复 |
...
| 画板 `Space` / `Ctrl+Space` / `←` `→` | 聚焦或恢复 / 强制适合完整画板 / 单图聚焦时切换相邻图片 |
| 画板 `F10` 或 `Alt` / `I` | 呼出隐藏顶部栏 / 打开或关闭临时检查器 |
