...
- 相似图片与文本找图：使用本地向量索引。
- PureRef 式多命名自由画板：默认只看内容，支持 Space 聚焦/精确恢复、自由平移缩放、共同选择变换、旋转裁剪、右键命令、隐藏顶部栏、窗口置顶、临时检查器、便签、背景和渐进高清。
- 发布级恢复：升级前完整性/空间/权限检查，数据库与设置备份，阶段标记，崩溃后继续验证或自动回滚。
- 脱敏诊断包：只导出日志、版本和白名单环境信息，不包含图片、完整提示词、密钥或设置。

## 安装与第一次运行

1. 解压 `FR_Imageprompt-M8-pureref-board-win-x64.zip`。
